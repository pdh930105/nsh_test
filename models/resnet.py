...
class ResNet_im(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNet_im, self).__init__()
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride)) # setting stride = 1 or 2
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, stride=1)) # the other layers always stride=1
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    model = ResNet_im(block, layers, **kwargs)
    if pretrained:
